...
export class ArcSingleNodeStack extends cdk.Stack {
    constructor(scope: cdkConstructs.Construct, id: string, props: ArcSingleNodeStackProps) {
        ...
        node.instance.addUserData(
            ...
            'dnf install -y --allowerasing docker wget curl jq amazon-cloudwatch-agent aws-cfn-bootstrap unzip',
            '',
            ...
            '',
            '# Download snapshot if requested (includes execution + consensus state with correct validator set)',
            ...
            '',
            '# Initialize consensus layer (only if no snapshot was downloaded)',
            snapshotDownload ? 'echo "Skipping consensus init - snapshot already provides consensus state"' : 'echo "Initializing consensus layer..."',
            snapshotDownload ? '' : `docker run --rm -v /data/arc-consensus:/home docker.cloudsmith.io/circle/arc-network/arc-consensus:${arcVersion} init --home /home`,
            '',
            ...
            'docker compose up -d',
            '',
            '# Wait for execution container to be running',
            'RETRIES=40',
            'COUNT=0',
