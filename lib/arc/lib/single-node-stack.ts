...
export class ArcSingleNodeStack extends cdk.Stack {
    constructor(scope: cdkConstructs.Construct, id: string, props: ArcSingleNodeStackProps) {
        ...
        node.instance.addUserData(
            ...
            'dnf install -y docker wget curl jq amazon-cloudwatch-agent aws-cfn-bootstrap unzip',
            '',
            ...
            'docker compose up -d',
            '',
            '# Wait for execution container to be running (image pull + init can take several minutes)',
            'RETRIES=40',
            'COUNT=0',
            'while [ $COUNT -lt $RETRIES ]; do',
            '  STATUS=$(docker inspect --format="{{.State.Status}}" arc-execution 2>/dev/null || echo "missing")',
            '  if [ "$STATUS" = "running" ] || [ "$STATUS" = "created" ]; then break; fi',
            '  COUNT=$((COUNT + 1))',
            ...
            'if [ "$EXECUTION_STATUS" = "running" ] || [ "$EXECUTION_STATUS" = "created" ]; then',
