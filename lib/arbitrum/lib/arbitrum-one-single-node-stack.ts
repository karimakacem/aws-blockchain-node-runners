...
export interface ArbitrumOneSingleNodeStackProps extends cdk.StackProps {
...
export class ArbitrumOneSingleNodeStack extends cdk.Stack {
    constructor(scope: cdkConstructs.Construct, id: string, props: ArbitrumOneSingleNodeStackProps) {
        ...
        if (!l1BeaconUrl || l1BeaconUrl === "") {
            throw new Error("L1_BEACON_URL is required. Please provide an Ethereum beacon endpoint.");
        }
        ...
        const instanceSG = new ec2.SecurityGroup(this, "security-group", {
            vpc: vpc,
            description: "Security group for Arbitrum One node",
        ...
        instanceSG.addIngressRule(
            ...
            "Arbitrum RPC (VPC only)"
        );

        // WebSocket port - VPC only
        instanceSG.addIngressRule(
            ec2.Peer.ipv4(vpc.vpcCidrBlock),
            ec2.Port.tcp(wsPort),
            "Arbitrum WebSocket (VPC only)"
        );

        // Metrics port - VPC only
        instanceSG.addIngressRule(
            ec2.Peer.ipv4(vpc.vpcCidrBlock),
            ec2.Port.tcp(metricsPort),
            "Arbitrum Metrics (VPC only)"
        );

        // Get IAM role from common stack
        const importedInstanceRoleArn = cdk.Fn.importValue(`ArbitrumArb1NodeInstanceRoleArn`);
        const instanceRole = iam.Role.fromRoleArn(this, "iam-role", importedInstanceRoleArn);

        // Get assets info from common stack
        const assetsBucket = cdk.Fn.importValue(`ArbitrumArb1AssetsBucket`);
        const assetsKey = cdk.Fn.importValue(`ArbitrumArb1AssetsKey`);
        ...
        const node = new SingleNodeConstruct(this, "arbitrum-one-node", {
        ...
        new cw.CfnDashboard(this, 'arbitrum-one-dashboard', {
        ...
        new cdk.CfnOutput(this, "node-instance-id", {
            ...
            description: "Arbitrum One node instance ID",
        });
