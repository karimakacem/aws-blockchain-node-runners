...
export interface AvalancheCommonStackProps extends cdk.StackProps {
    snapshotType: configTypes.SnapshotType;
}

export class AvalancheCommonStack extends cdk.Stack {
    AWS_STACKNAME = cdk.Stack.of(this).stackName;
    AWS_ACCOUNT_ID = cdk.Stack.of(this).account;
    AWS_REGION = cdk.Stack.of(this).region

    constructor(scope: cdkConstructs.Construct, id: string, props: AvalancheCommonStackProps) {
        ...
        instanceRole.addToPolicy(
            new iam.PolicyStatement({
                resources: [
                    `arn:aws:autoscaling:${region}:${this.AWS_ACCOUNT_ID}:autoScalingGroup:*:autoScalingGroupName/avalanche-*`,
                ],
                actions: ["autoscaling:CompleteLifecycleAction"],
            })
        );

        // Add EC2 permissions for volume attachment
        instanceRole.addToPolicy(
            new iam.PolicyStatement({
                resources: ["*"],
                actions: [
        ...
        if (props.snapshotType === "s3") {
            const snapshotsBucket = new SnapshotsS3BucketConstruct(this, `snapshots-s3-bucket`, {
                bucketName: `${this.AWS_STACKNAME}-${this.AWS_ACCOUNT_ID}-${this.AWS_REGION}`.toLowerCase(),
            });
            ...
            new cdk.CfnOutput(this, "Snapshot Bucket Name", {
                ...
                exportName: "AvalancheNodeSnapshotBucketName",
            });
        }

        new cdk.CfnOutput(this, "Instance Role ARN", {
            value: instanceRole.roleArn,
            exportName: "AvalancheNodeInstanceRoleArn",
        });
        ...
        nag.NagSuppressions.addResourceSuppressions(
            ...
            [
                ...
                {
                    id: "AwsSolutions-S1",
                    reason: "No access log needed for storing nodes state as it is public data",
                },
            ],
            true
        );
