...
export interface AvalancheSyncNodeStackProps extends cdk.StackProps {
...
export class AvalancheSyncNodeStack extends cdk.Stack {
    constructor(scope: cdkConstructs.Construct, id: string, props: AvalancheSyncNodeStackProps) {
        ...
        // Get snapshot bucket name if using S3 snapshots
        ...
        node.instance.addUserData(
            ...
            'dnf install -y wget jq amazon-cloudwatch-agent cronie aws-cfn-bootstrap',
            '',
            ...
            'EOF',
            '',
            '# Create environment file for sync-node',
            ...
            'EOF',
            '',
            '# Install s5cmd for fast S3 sync',
            snapshotType === "s3" ? 'cd /tmp && wget -q https://github.com/peak/s5cmd/releases/download/v2.2.2/s5cmd_2.2.2_Linux-64bit.tar.gz' : '',
            snapshotType === "s3" ? 'tar -xzf s5cmd_2.2.2_Linux-64bit.tar.gz' : '',
            snapshotType === "s3" ? 'mv s5cmd /usr/local/bin/' : '',
            snapshotType === "s3" ? 'chmod +x /usr/local/bin/s5cmd' : '',
            '',
            '# Download and set up storage scripts',
            `aws s3 cp s3://${asset.s3BucketName}/${asset.s3ObjectKey} /tmp/assets.zip`,
            'cd /tmp && unzip -q assets.zip',
            'mkdir -p /opt/avalanche/storage /opt/avalanche/sync-checker',
            snapshotType === "s3" ? 'cp /tmp/storage/*.sh /opt/avalanche/storage/' : '',
            'cp /tmp/sync-checker/*.sh /opt/avalanche/sync-checker/',
            'chmod +x /opt/avalanche/storage/*.sh /opt/avalanche/sync-checker/*.sh',
            '',
            '# Set up sync-checker cron job (runs every 15 minutes)',
            'systemctl enable crond',
            'systemctl start crond',
            '(crontab -l 2>/dev/null; echo "*/15 * * * * /opt/avalanche/sync-checker/syncchecker.sh >> /var/log/avalanche/syncchecker.log 2>&1") | crontab -',
            '',
            '# Start service',
            ...
            '',
            '# Wait for avalanchego to become active',
            'RETRIES=40',
            'COUNT=0',
            'while [ $COUNT -lt $RETRIES ]; do',
            '  STATUS=$(systemctl is-active avalanchego 2>/dev/null)',
            '  if [ "$STATUS" = "active" ]; then break; fi',
            '  if [ "$STATUS" = "failed" ]; then break; fi',
            '  COUNT=$((COUNT + 1))',
            '  echo "Waiting for avalanchego... ($COUNT/$RETRIES) status=$STATUS"',
            '  sleep 15',
            'done',
            '',
            '# Signal CloudFormation',
            'if [ "$(systemctl is-active avalanchego)" = "active" ]; then',
            `  cfn-signal --stack ${STACK_NAME} --resource ${node.nodeCFLogicalId} --region ${REGION} --success true`,
            '  echo "✓ Setup complete"',
            'else',
            '  journalctl -u avalanchego -n 20 --no-pager',
